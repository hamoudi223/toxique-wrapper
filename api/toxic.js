...
// Modèles à essayer dans l'ordre. Si le premier est surchargé (503) ou
// indisponible (404/429), on bascule automatiquement sur le suivant.
const MODEL_FALLBACKS = ["gemini-3.5-flash", "gemini-2.5-flash", "gemini-2.0-flash"];

function sleep(ms) {
  return new Promise((r) => setTimeout(r, ms));
}

async function generateWithFallback(ai, systemInstruction, text) {
  let lastError;
  ...
  for (const modelName of MODEL_FALLBACKS) {
    ...
    // 2 tentatives par modèle avec un petit backoff avant de passer au suivant
    for (let attempt = 0; attempt < 2; attempt++) {
      try {
        ...
        if (isOverloaded || isRateLimited) {
          await sleep(600 * (attempt + 1)); // backoff avant retry
          continue;
        }
        // erreur non transitoire (clé invalide, 400, etc.) -> inutile de retry
        break;
      }
    }
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const aiResponse = await generateWithFallback(ai, systemInstruction, text);
